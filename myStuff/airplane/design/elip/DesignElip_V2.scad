...
//SmoothCylinder(radius=eSpan, height=wingThick*3, smooth_rad=1);

eSpan = 120;
translate([eSpan,0,0]) {
difference() {
    SmoothCylinder(radius=eSpan, height=wingThick*3, smooth_rad=wingThick);
    translate([3,0,wingThick])cylinder(r=eSpan, h=wingThick*3+1);
    translate([0,0,-1])scale([.6,1.1,1])cylinder(r=eSpan, h=4);
    translate([0,-eSpan,-1])cube([eSpan,eSpan*2,4]);
}
}
