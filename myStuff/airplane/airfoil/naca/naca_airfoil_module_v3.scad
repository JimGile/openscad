...
module naca_airfoil_holes(chord,t,n,num_holes=10,flat=false) {
    top_chord_line_list = naca_top_chord_line_list(t=t,n=num_holes*.8);
    if (flat == true) {
    ...
    difference() {
        ...
        for (chord_list = top_chord_line_list) {
            if (flat == true) {
                if (chord_list[1]*c/2 > .2) {
                    translate([chord_list[0]*c, chord_list[1]*c/2, 0]) 
                    circle((chord_list[1]*c/2)*2/3);
                }
            } else {
                if (chord_list[1]*c/2 > .4) {
                    translate([chord_list[0]*c, 0, 0]) 
                    circle((chord_list[1]*c/2)*1.3);
                }
            }
        }
    }
...
module naca_wing_holes(span,chord,t,n,num_holes,scale,flat,center=false) {
    linear_extrude(height=span,center=center,scale=scale,slices=10,convexity=3) {
        rotate([0,0,180])
        naca_airfoil_holes(chord=chord,t=t,n=n,num_holes=num_holes,flat=flat);
    }
}

module naca_wing_supports(span,chord,t,n,num_holes,scale,flat,center=false) {
    z=chord*t;
    width = .5;
    step = 5;
    //End Caps
    difference() {
        ...
        translate([-(chord+1),width,-z/4]) cube([chord+2,span-(2*width),z]);
    }
    difference() {
        rotate([-90,0,0])
        naca_wing_holes(span=span,chord=chord,t=t,n=n,num_holes=num_holes,scale=scale,flat=flat);
        for (y=[0:step:span-step]) {
            translate([-(chord+1),y+width/2,-z/4]) cube([chord+2,step-width,z]);
        }
    }
}
...
module naca_wing_hollow_supports(span,chord,t,n,h,num_holes,scale,flat,center=false) {
    ...
    naca_wing_supports(span=span,chord=chord,t=t,n=n,num_holes=num_holes,scale=scale,flat=flat);
}
...
//naca_airfoil_holes(chord=c,t=t,n=n,num_holes=holes,flat=flat);
