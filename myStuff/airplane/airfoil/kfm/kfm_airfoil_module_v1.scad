...
module kfm0_airfoil(chord,holes=true) {
    thick=chord*.04;
    if (holes==true) {
...
module kfm0_wing_extrude(span,chord,scale=[1,1],holes=true,center=false) {
    rotate([0,-90,180])
    linear_extrude(height=span,center=center,scale=scale,slices=15,convexity=3) {
        kfm0_airfoil(chord,holes=holes);
    }
}

module kfm0_wing_endcaps(span,chord,scale=[1,1],center=false) {
    z=chord*.2;
    width = 1;
    difference() {
        kfm0_wing_extrude(span=span,chord=chord,scale=scale,holes=false);
        translate([width,-width,-((z/2)+1)]) cube([span-(2*width),chord+2,z+2]);
    }
}

module kfm0_wing(span,chord,scale=[1,1],center=false) {
    kfm0_wing_extrude(span,chord,scale);
    kfm0_wing_endcaps(span,chord,scale);
}

module kfm0_wing_assembly(span,chord,scale=[1,1]) {
    kfm0_wing(span,chord,scale);
    mirror([1,0,0])
    kfm0_wing(span,chord,scale);
}
