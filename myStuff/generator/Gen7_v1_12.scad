...
module barMagDisk() {
    border = mag1_hole_r;
    mag1_disk_r = central_r + border+11.5;
    w=mag1_w;
...
module barMagCoilOut(r,b,x=30,y=10,z=20) {
    offset = x/2 + y/2;
...
module barMagCoilOutlines(r,b,x=30,y=10,z=20) {
    sectors = num_holes;
    sector_degrees = 360 / sectors;
    for(sector = [1 : 2]) {
        angle = sector_degrees * sector;
...
module barMagCoilIn(r,b,sector,x=30,y=10,z=20) {
    r_in = 1;
    x_off = x/2;
    y_off = y/2 + r_in;
    ...
    if (sector == 2) {
        translate([r-(x/2)-b,0,0]){
            #translate([x_off,-y_off,0])cylinder(h=z,r=r_in,center=true);
            %translate([0,-y_off,0])cube([x, r_in*2, z], center=true);
            #translate([-x_off,-y_off,0])cylinder(h=z,r=r_in,center=true);
        }
    } else {
        translate([r-(x/2)-b,0,0]){
...
module barMagCoilInlines(r,b,x=30,y=10,z=20) {
    sectors = num_holes;
    sector_degrees = 360 / sectors;
    for(sector = [1 : 2]) {
        angle = sector_degrees * sector;
