/* 
   GEN 11 v5.6: THE INTEGRATED TRIPLE-CHECK MASTER
   - COMPONENTS: Hex-Lock Rotor, Bearing-Ready Stators, Flux-Return Plates, Spacers, Winder.
   - VERIFICATION: Added "verify_alignment" mode to see Coil-to-Washer overlap.
   - LOCKING: Central 13.5mm Hex-Nut Trap for M8 Rod.
   - TOLERANCE: Unified magnet_center_r across all parts for modular alignment.
*/
...
verify_alignment    = true; // Set to TRUE to see the "X-Ray" alignment check
...
module renderFullAssembly() {
    arm_dist = (rotor_total_h/2) + air_gap; 
    color("Silver") cylinder(h=120, r=axel_r, center=true, $fn=40);
    color("Gold", 0.8) bar_rotor();
    color("DimGray") {
        translate([0,0, rotor_total_h/2 - 2]) rotate([0,0,30]) m8_nut();
        translate([0,0, -rotor_total_h/2 + 2]) rotate([0,0,0]) m8_nut();
    }
    ...
    flux_h = washer_flux_depth + flux_plate_floor;
...
module bar_rotor() {
    difference() {
        cylinder(h=rotor_total_h, r=rotor_r, center=true, $fn=120);
        ...
        cylinder(h=rotor_total_h + 2, r=nut_hex_dist/2 / cos(30), center=true, $fn=6);
    }
}
...
module bar_stator() {
    difference() {
        ...
        translate([0, 0, 1.5]) cylinder(h=stator_h, r=13.5, $fn=80); 
