...
//innerCoilHolderShell();
//coilHolderTopInsert();
coilHolderBottom();
rotate([0,0,sector_degrees*2])coilHolderTop();

//outerCoilHolder();

//rotate([0,0,(sector_degrees/2)])
//translate([coil_holder_center_r,0,0]) cylinder(h=coil_w,r=2);
...
module coilHolderTop() {
    difference() {
        ...
        coilHolderCenterCylinder(z=coil_cap_w);
        coilHolderCenterBoltHole();
    }
}
...
module coilHolderTopInsert(h=coil_holder_top_insert_h, z=coil_cap_w) {
    translate([0,0,z]) {
        linear_extrude(height=h, slices=1) {
            offset(r=-(coil_holder_inner_shell_t+0.1)) {
                innerCoilHolder2d();
            }
        }
    }
...
module coilHolderCap(w=coil_cap_w) {
    translate([0,0,w/2])outerCoilHolder(w=w);
}

module coilHolderCenterCylinder(h=coil_w, r=coil_holder_bolt_r+1.5, z=0) {
    rotate([0,0,(sector_degrees/2)])
...
module coilHolderCenterBoltHole() {
    coilHolderCenterCylinder(h=coil_w*3,r=coil_holder_bolt_r,z=-coil_w);
}



//#innerCoilHolder();
//scale([.9,.9,1])innerCoilHolder();
//%outerCoilHolder();
//coilDisk2(diskNum=1, bearing=true);
...
module coilDisk2(w=coil_w, diskNum=1, wires=true, bearing=false) {
    x = base_d-1.5;
    y = x;
    small_hole_r = 1;
