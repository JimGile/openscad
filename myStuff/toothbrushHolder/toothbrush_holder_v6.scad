...
module fullRender() {
    ...
    difference() {
        union() {
            base();
            translate([x_offset,0,z_offset]) screwPlate();            
        }
        // Remove center screw hole
        translate([x_offset,0,0]) screwHole();
    }
}

module base() {
    // Render holders and connector
...
module holderConnector() {
    ...
    translate([spacing_r-thickness,0,0])
        rotate([0,0,180])
        halfCubeSmoothHole(spacing_r, thickness, holder_h);
}
