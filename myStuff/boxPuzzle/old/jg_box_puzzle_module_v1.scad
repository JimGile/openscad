...
module box_piece(piece_num, thickness, edge_notch_list, boxLetters) {
    slop = .05;
    ...
    //Translations List
    t = [
        [-slop*2,           side_len-thickness, -slop/2], //Top
        [thickness-slop/2,  -slop/2,            -slop/2],   //Left
        [-slop/2,           -slop/2,            -slop/2],            //Bot
        [side_len,          -slop/2,            -slop/2]            //Right
    ];
    //Rotations List
    r = [
        [0,0,0],    //Top
...
module notched_edge(edge_num, thickness, notch_list, slop) {
    union() {
        for (i=[0:len(notch_list)-1]) {
            ...
            cube([notch_list[i]*(thickness+slop*2), thickness+slop*2, thickness+slop*2]);
        }
    }
}

module notched_edge_old2(edge_num, thickness, notch_list, slop) {
    union() {
        for (i=[0:len(notch_list)-1]) {
            translate([i*thickness-(slop), -slop/2, -slop/2])
            #cube([notch_list[i]*(thickness+slop), thickness+slop, thickness+slop]);
        }
    }
}

module notched_edge_old(edge_num, thickness, notch_list) {
    slop = .015;
    scale([1+slop,1+slop,1+slop]) {
        union() {
            for (i=[0:len(notch_list)-1]) {
                translate([i*thickness, -slop, -thickness/2])
                #cube([notch_list[i]*thickness, thickness, thickness*2]);
            }
        }
    }
}